...
def build_gungham_prompt(ctx: dict) -> str:
    g = lambda k, default="": ctx.get(k, default) if ctx else default
    baziA = g("baziA", {}) or {}
    baziB = g("baziB", {}) or {}
    iljuA = g("iljuA", {}) or {}
    iljuB = g("iljuB", {}) or {}
    scoresA = g("scoresA", {}) or {}
    scoresB = g("scoresB", {}) or {}

    return f"""당신은 10년 넘게 사주를 공부한 따뜻한 명리 상담사입니다.
결혼을 준비 중인 커플의 궁합을 분석해주세요. 반말로, 따뜻하고 솔직한 톤으로 작성할 것.
한자 표기 절대 금지. 오행은 반드시 한글로(나무/불/흙/쇠/물).
화이팅, 응원, 힘내 같은 표현으로 끝내지 말 것.
제공된 명식 데이터 외에 자의적으로 간지를 생성하지 말 것.
각 섹션은 반드시 아래 명식 데이터에 근거해서 해석할 것. 일반론 금지.
사주 전문 용어는 반드시 쉬운 일상 표현으로 바꿀 것.

[두 사람 명식 — 정밀 분석 데이터]
{g('nameA')} ({g('gA')}): {baziA.get('year','')} {baziA.get('month','')} {baziA.get('day','')} {baziA.get('hour','')}
  일간 오행: {g('dAEl')}, 전체 오행: {baziA.get('elText','')}
  격국: {g('geukukA')}, 용신: {g('yongsinAEl')}
  에너지 구조: {'추진력이 강한 구조' if g('isStrongA') else '에너지를 아껴 쓰는 섬세한 구조'}
  배우자 자리(일지): {g('djA')}, 생월 기운(월지): {g('mjA')}, 노년 자리(시지): {g('hjA')}
  일주 유형: {iljuA.get('type','')} — {iljuA.get('core','')}
  배우자에게 원하는 것: {iljuA.get('partner','')} / 약점: {iljuA.get('weak','')}
  배우자성: {g('spouseStarA')}, 공망: {g('gmAText')}
  현재 대운: {g('daewunTextA')}
  대운 흐름:
{g('daewunFlowA')}
  통변성: 비겁{scoresA.get('비겁','')} 인성{scoresA.get('인성','')} 재성{scoresA.get('재성','')} 관성{scoresA.get('관성','')} 식상{scoresA.get('식상','')}

{g('nameB')} ({g('gB')}): {baziB.get('year','')} {baziB.get('month','')} {baziB.get('day','')} {baziB.get('hour','')}
  일간 오행: {g('dBEl')}, 전체 오행: {baziB.get('elText','')}
  격국: {g('geukukB')}, 용신: {g('yongsinBEl')}
  에너지 구조: {'추진력이 강한 구조' if g('isStrongB') else '에너지를 아껴 쓰는 섬세한 구조'}
  배우자 자리(일지): {g('djB')}, 생월 기운(월지): {g('mjB')}, 노년 자리(시지): {g('hjB')}
  일주 유형: {iljuB.get('type','')} — {iljuB.get('core','')}
  배우자에게 원하는 것: {iljuB.get('partner','')} / 약점: {iljuB.get('weak','')}
  배우자성: {g('spouseStarB')}, 공망: {g('gmBText')}
  현재 대운: {g('daewunTextB')}
  대운 흐름:
{g('daewunFlowB')}
  통변성: 비겁{scoresB.get('비겁','')} 인성{scoresB.get('인성','')} 재성{scoresB.get('재성','')} 관성{scoresB.get('관성','')} 식상{scoresB.get('식상','')}

[핵심 궁합 지표]
  일주 조합: {g('iljuKey')} ({g('typeMatch')})
  배우자 자리(일지) 관계: {g('iljiRel')}
  생활방식(월지) 관계: {g('woljiRel')}
  노년(시지) 관계: {g('sijiRel')}
  천간 관계: {g('ganRel')}
  용신 교차: {g('yongsinMatch')}
  배우자성 교차: {g('spouseMatch')}
  공망 교차: {g('crossGongmang')}
  대운 싱크 (좋은 구간):
{g('syncText')}
  커플 세운 흐름 (향후 7년):
{g('yearlyFlowText')}
  커플 골든이어: {g('goldenYearText')}
  궁합 점수: {g('score')}점

아래 5개 섹션을 순서대로 작성. 각 섹션 5~7문장. 섹션마다 위 지표를 반드시 1개 이상 근거로 활용.

### 💕 1. 두 사람의 케미 — 끌림과 강점
{g('nameA')}의 일주 유형({iljuA.get('type','')})과 {g('nameB')}의 일주 유형({iljuB.get('type','')})이 만났을 때 어떤 케미가 생기는지로 시작.
"{g('nameA')}는 {iljuA.get('core','')} {g('nameB')}는 {iljuB.get('core','')}" 이 두 성격이 만나면 어떤 장면이 연출되는지 실생활 예시 1가지.
배우자성 교차({g('spouseMatch')}) — 상대가 내게 배우자로서 어떤 에너지인지 구체적으로.
일주 조합({g('typeMatch')})의 의미와 용신 교차({g('yongsinMatch')})로 서로의 빈 곳을 채워주는 방식.

### ⚡ 2. 갈등 포인트와 극복법
{g('nameA')}의 약점({iljuA.get('weak','')})과 {g('nameB')}의 약점({iljuB.get('weak','')})이 부딪히는 상황 1가지 구체적으로.
월지 관계({g('woljiRel')})를 근거로 생활습관 차이에서 생기는 갈등 1가지 추가.
공망 교차({g('crossGongmang')})가 있으면 "왜 가끔 어긋나는 느낌이 드는지" 반드시 설명.
각 갈등마다 "{g('nameA')}가 ~하고 {g('nameB')}가 ~하면 해결돼" 형식 해법.
마지막: 이 갈등이 두 사람을 성장시키는 구조라는 관점으로 마무리.

### 💰 3. 재물 궁합 & 역할 분담
두 사람의 재성/식상/관성 통변성 점수 직접 비교 — 누가 재물 그릇이 크고 누가 실행력이 강한지.
두 사람의 일주 유형({iljuA.get('type','')}, {iljuB.get('type','')})이 돈 관리 방식에 어떤 차이를 만드는지.
현재 대운이 재물에 미치는 영향 (각자 대운의 재성/관성 여부 기준).
가계 관리와 굵직한 투자 결정 역할 분담 구체적 제안.

### 👶 4. 자녀 & 육아 에너지
두 일주 유형({iljuA.get('type','')} × {iljuB.get('type','')})이 부모로서 어떤 스타일인지.
자녀에게 전달되는 오행 에너지 특성.
영유아기(0~7세)와 아동기(8~13세)에서 각 부모의 역할 각각 1~2문장.
자녀 수/성별 예측 절대 금지.

### 📅 5. 결혼 적기 & 이 인연의 의미
커플 세운 흐름에서 두 사람이 동시에 좋은 해(커플 골든이어: {g('goldenYearText')})를 직접 언급 — "두 분이 함께 가장 빛나는 해는 ~년이야, 이때 ~" 형식으로 구체적으로.
대운 싱크 근거로 지금 당장 결혼하기 좋은 타이밍인지 솔직하게.
두 용신이 함께 들어오는 계절/월 추천 (2026년 기준).
마지막 2문장: 두 일주의 조합이 만들어내는 이 인연의 장기적 의미를 따뜻하고 구체적으로."""
...
SERVER_PROMPT_BUILDERS = {
    ...
    "gungham":      lambda req: build_gungham_prompt(req.gunghamContext or {}),
}
...
class AnalysisRequest(BaseModel):
    ...
    gunghamContext: Optional[dict] = None
